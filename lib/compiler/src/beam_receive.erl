...
opt([{call_ext,Arity,{extfunc,erlang,Name,Arity}}=I|Is0], D, Acc) ->
	...
	true ->
	    %% The call can create a brand new reference. Now
...
opt_recv([{label,L}=Lbl,{loop_rec,{f,Fail},_}=Loop|Is], D, R0, Acc) ->
...
opt_recv([I|Is], D, R0, Acc) ->
    R = opt_update_regs(I, R0),
	...
	true ->
	    ...
	    no;
	false ->
	    opt_recv(Is, D, R, [I|Acc])
    end;
opt_recv([], _, _, _) -> no.

opt_update_regs({block,Bl}, R) ->
    opt_update_regs_bl(Bl, R);
opt_update_regs({call,_,_}, R) ->
    regs_kill_not_live(0, R);
opt_update_regs({call_ext,_,_}, R) ->
    regs_kill_not_live(0, R);
opt_update_regs({kill,Y}, R) ->
    regs_kill([Y], R);
opt_update_regs(send, R) ->
    regs_kill_not_live(0, R);
opt_update_regs({'catch',_,_}, R) -> R;
opt_update_regs({catch_end,_}, R) -> R;
opt_update_regs({label,_}, R) -> R;
opt_update_regs({try_end,_}, R) -> R;
opt_update_regs(_I, _R) ->
...
opt_ref_used_bl([{set,Ds,Ss,_}|Is], Regs0) ->
	...
	false ->
	    ...
	    %% are not dependent on message being matched.
	    Regs = regs_kill(Ds, Regs0),
...
%%  Test whethere the register set is empty.
